        ...
        <div class="col-md-10 col-md-offset-1">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
					...
					<tr><td>Интербанк</td><td>{{$results->USD->interbank->buy}}</td><td>{{$results->EUR->interbank->buy}}</td><td>{{$results->RUB->interbank->buy}}</td></tr>
					<tr><td>НБУ</td><td>{{$results->USD->nbu->buy}}</td><td>{{$results->EUR->nbu->buy}}</td><td>{{$results->RUB->nbu->buy}}</td></tr>
					</table><p>
					
					<table class="table">
					<caption>Продажа</caption>
					<tr><th>Банк</th><th>USD</th><th>EUR</th><th>RUB</th></tr>
					<tr><td>Интербанк</td><td>{{$results->USD->interbank->sell}}</td><td>{{$results->EUR->interbank->sell}}</td><td>{{$results->RUB->interbank->sell}}</td></tr>
					<tr><td>НБУ</td><td>{{$results->USD->nbu->sell}}</td><td>{{$results->EUR->nbu->sell}}</td><td>{{$results->RUB->nbu->sell}}</td></tr>
					</table>
					<p>
...
$('document').ready(function()
{
	$('.compare').click(function()
	{
	  ...
	  @for($i=0; $i < 30 ; $i++)
	  nbu_usd_b[{{$i}}] = {{$result[$i]->USD->nbu->buy}};
	  nbu_usd_s[{{$i}}] = {{$result[$i]->USD->nbu->sell}};
	  nbu_eur_b[{{$i}}] = {{$result[$i]->EUR->nbu->buy}};
	  nbu_eur_s[{{$i}}] = {{$result[$i]->EUR->nbu->sell}};
	  nbu_rub_b[{{$i}}] = {{$result[$i]->RUB->nbu->buy}};
	  nbu_rub_s[{{$i}}] = {{$result[$i]->RUB->nbu->sell}};
	  int_usd_b[{{$i}}] = {{$result[$i]->USD->interbank->buy}};
	  int_usd_s[{{$i}}] = {{$result[$i]->USD->interbank->sell}};
	  int_eur_b[{{$i}}] = {{$result[$i]->EUR->interbank->buy}};
	  int_eur_s[{{$i}}] = {{$result[$i]->EUR->interbank->sell}};
	  int_rub_b[{{$i}}] = {{$result[$i]->RUB->interbank->buy}};
	  int_rub_s[{{$i}}] = {{$result[$i]->RUB->interbank->sell}}; 
	  date_n[{{$i}}] = '{{$n_date[$i]}}';
		@endfor
